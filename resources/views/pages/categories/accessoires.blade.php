  ...
  <div class="container">
    ...
    <div class="card">
      <div class="image-container">
        <a href="#" onclick="refreshPageAndGo('{{ route('detailsP', ['id' => $accessoire->id]) }}');">
          <img src="{{ asset($accessoire->img) }}" alt="Product Image">
        </a>
      </div>
      <h2 class="title">{{ $accessoire->title }}</h2>
      <div class="price-container">
        <span class="price">{{ $accessoire->prix }} DH</span>
        @php
        $lastPrice = $accessoire->prix * 1.15;
        @endphp
      ...
      <div class="button-container">
        @if (Route::has('login'))
            @auth
            <form action="{{ route('panier', $accessoire->id) }}" method="get">
              @csrf
